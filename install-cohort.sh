...
install_completion() {
  ...
  # Wire every shell that already has an rc file. A machine with none — a fresh
  # container, a new account — still gets its login shell wired, so completion
  # is never installed and then left unsourced.
  local -a shells=()
  for sh in bash zsh; do
    [[ -f $(rc_for "$sh") ]] && shells+=("$sh")
  done
  if [[ ${#shells[@]} -eq 0 ]]; then
    case ${SHELL##*/} in
      zsh) shells=(zsh) ;;
