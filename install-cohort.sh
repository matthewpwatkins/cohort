...
is missing.
  ...
  ./install-cohort.sh --no-claude  skip the Claude Code check
  ./install-cohort.sh --yes        do not ask before installing anything
...
# Unlike tmux this is not a gate. cohort installs perfectly well before Claude
# Code does, and someone may be scripting a machine image where claude arrives
# later, so a no here costs nothing but a note.
install_claude() {
  section "Claude Code"
  if command -v claude >/dev/null; then
    status present "$(command -v claude)"
    return 0
  fi

  status missing "cohort runs Claude Code sessions"
  local answer=0
  confirm "Install it with: $CLAUDE_INSTALL ?" || answer=$?
  if (( answer == 2 )); then
    status skipped "no terminal to ask on — re-run with --yes, or install it yourself"
    return 1
  elif (( answer )); then
    status declined "install it when you are ready"
    return 1
  fi

  if ! command -v curl >/dev/null; then
    status failed "need curl to fetch the installer"
    return 1
  fi
  status installing "$CLAUDE_INSTALL"
  curl -fsSL https://claude.ai/install.sh | bash || true
  hash -r 2>/dev/null || true

  if command -v claude >/dev/null; then
    status installed "$(command -v claude)"
    return 0
  fi
  # Its installer picks its own location, which this shell may not search yet.
  if [[ -x $HOME/.local/bin/claude ]]; then
    status installed "$HOME/.local/bin/claude"
    note "not on this shell's PATH yet — open a new terminal"
    return 0
  fi
  status failed "install it yourself: $CLAUDE_INSTALL"
  return 1
}
...
if [[ $MODE == install ]]; then
  ...
  if (( ! CLAUDE_OK )); then
    printf '\n  Claude Code is not installed, so no session will start yet:\n'
    note "$CLAUDE_INSTALL"
  fi
  printf '\n'
